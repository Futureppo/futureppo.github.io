...
因为反重力IDE限制MCP工具数量100个，所以我在使用 [jshookmcp](https://github.com/vmoranv/jshookmcp) 的时候，被迫开启search模式，效果很差。
...
Error: Cannot enable more tools because it would exceed the limit of 100.
```
反重力基于vscode，前端 UI 层本质上是打包后的 Web 应用。报错信息 `Cannot enable more tools because it would exceed the limit of 100` 大概率直接存在于前端代码中。
...
这么简单？出来了？我直接把 `yy=100` 改成 `yy=114514`，保存文件。但事情没有这么简单，重新打开 IDE 后，它会检测到 JS 文件被篡改，然后左下角报错损坏，关掉后没影响。因为 `product.json` 中记录了该文件的校验值。所以修补后还需要重新计算修改后文件的哈希，所以我更新了 `product.json` 的校验值。
...
虽然前端不再弹出损坏提示了，但实际还是无法启用。这说明数量检查不仅在前端做了，后端也有独立的校验逻辑。
...
这是一个编译好的二进制文件，直接丢到IDA里分析。丢进去，先洗个澡，等分析结束。
...
洗完澡回来啦~继续
### 搜索思路
已经知道限制值是 100。在 x86-64 架构下，数值比较通常使用 `cmp` 指令。100 的十六进制是 `0x64`。那么在二进制中，限制检查大概率表现为某种形式的：
...
问题在于，`0x64` 这个值在一个几百 MB 的二进制文件中可能出现成百上千次。需要足够具体的特征码才能精确定位。
...
共 6 字节，这个序列已经足够具体，在整个二进制中的匹配数量通常只有个位数。
...
<details>
<summary>折叠/展开查看完整伪代码 (sub_14024E600)</summary>
...
<details>
<summary>折叠/展开查看完整伪代码 (sub_1416EDCC0)</summary>
...
<details>
<summary>折叠/展开查看完整伪代码 (sub_1416EE3C0)</summary>
...
观察上下文，这个函数在处理长度为 2 和 4 的数组，并且比较了 1114111（十六进制 0x10FFFF，Unicode 最大码点）以及 9、11 这些值。这是一个文本处理或者正则引擎中的字符类优化逻辑，> 100 只是判断字符范围区间大小是否超过阈值来决定数据结构表示方式，跟工具数量限制无关。
...
但有一个细节：`jle near` 的操作码是双字节 `0F 8E`，而 `jmp near` 的操作码是单字节 `E9`。两者后面都跟 4 字节的相对偏移。操作码少了 1 字节，需要用 `nop`（`0x90`）来填充，否则后面的偏移量会错位导致指令流损坏。
...
工具使用 Go 实现，零外部依赖，构建出来就是一个单文件可执行程序，覆盖 Windows / Linux / macOS 三平台。核心逻辑：
...
项目地址：[GitHub - Futureppo/antigravity_bypass](https://github.com/Futureppo/antigravity_bypass)
